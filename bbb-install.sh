...
install_greenlight(){
  ...
  SECRET_KEY_BASE=$(docker run --rm themaqs/gl:latest bundle exec rake secret)

  if [ ! -f ~/greenlight/.env ]; then
    docker run --rm themaqs/gl:latest cat ./sample.env > ~/greenlight/.env
  fi
  ...
  if [ ! -f /etc/bigbluebutton/nginx/greenlight.nginx ]; then
    docker run --rm themaqs/gl:latest cat ./greenlight.nginx | tee /etc/bigbluebutton/nginx/greenlight.nginx
...
HERE
  ...
  if [ ! -f ~/greenlight/docker-compose.yml ]; then
    docker run --rm themaqs/gl:latest cat ./docker-compose.yml > ~/greenlight/docker-compose.yml
  fi
  ...
  if docker ps | grep -q iasbs-db; then
    docker rm -f iasbs-db
  fi
  if docker ps | grep -q iasbs-gl; then
    docker rm -f iasbs-gl
  fi

  if ! docker ps | grep -q iasbs; then
    ...
    erdir=$erdir/usr/src/app/vendor/bundle/ruby/2.7.0/gems;
    esmal=`grep -r "jpegPhoto" $erdir | cut -d':' -f1`;
    sed -i "s/\[:jpegPhoto]/[:jpegPhoto],'pre_name' => [:eduPersonNickname],'iasbs_role' => [:eduPersonEntitlement]/g" $esmal;
    esmal=`grep -r '#0076ff' $erdir/* | cut -d':' -f1`;
