...
function loadMetadata() {
  ...
  xmldoc = parser.parseFromString(htmlDecode(metadataXML),"text/xml");
	processMetadataXML(xmldoc);
};

function processMetadataXML(response) {
  logger.info("==Processing metadata.xml");
  metadataXMLContent = response;
...
function loadData() {  
  parser = new DOMParser();
  xmldoc = parser.parseFromString(htmlDecode(shapesSVG),"image/svg+xml");
  processShapesSVG(xmldoc);
  
  parser = new DOMParser();
  xmldoc = parser.parseFromString(htmlDecode(panzoomsXML),"text/xml");
  processPanzoomsXML(xmldoc);

  parser = new DOMParser();
	xmldoc = parser.parseFromString(htmlDecode(cursorXML),"text/xml");
  processCursorXML(xmldoc);

  parser = new DOMParser();
	xmldoc = parser.parseFromString(htmlDecode(deskshareXML),"text/xml");
  processDeskshareXML(xmldoc);
};

function processPanzoomsXML(response) {
  logger.info("==Processing panzooms.xml");
  let panelements = response.getElementsByTagName("recording");
  let panZoomArray = panelements[0].getElementsByTagName("event");
  let viewBoxes = response.getElementsByTagName("viewBox");
...
function processCursorXML(response) {
  ...
  let curelements = response.getElementsByTagName("recording");
  let cursorArray = curelements[0].getElementsByTagName("event");
  let coords = response.getElementsByTagName("cursor");
...
function processShapesSVG(response) {
  ...
  $(svgobj).html(response.lastChild.cloneNode(true));
...
function processDeskshareXML(response) {
  ...
  let deskshareElements = response.getElementsByTagName("recording");
